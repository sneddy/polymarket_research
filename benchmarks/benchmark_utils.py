...
def build_multi_horizon_terminal_dataset(
    ...
    *,
    ...
    df = df.sort_values(["end_date", "market_id", "horizon_hours"], kind="stable").reset_index(drop=True)
    return df
...
def time_based_split(
    ...
    *,
    ...
    split_idx = max(1, int(len(work) * (1.0 - test_fraction)))
...
def _normalize_market_frame(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = out.dropna(subset=["created_at", "end_date"]).reset_index(drop=True)
    return out
