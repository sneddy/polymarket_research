...
def extract_primary_event_fields(events_payload: Any) -> dict[str, str | None]:
    ...
    return {
        ...
        "event_series_slug": text_value(event.get("seriesSlug")),
    }
...
def upsert_market_universe(conn: sqlite3.Connection, markets_df: pd.DataFrame) -> int:
    ...
    parsed_prices = work.get("outcome_prices", pd.Series(index=work.index, dtype=object)).map(parse_binary_prices)
    parsed_outcomes = work.get("outcomes", pd.Series(index=work.index, dtype=object)).map(parse_binary_outcomes)
    work["final_outcome"] = [
        resolved_outcome_from_prices(prices, outcomes) if prices is not None and outcomes is not None else None
        for prices, outcomes in zip(parsed_prices, parsed_outcomes, strict=False)
    ]
