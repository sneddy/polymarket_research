...
def camel_to_snake(name: str) -> str:
    """Convert a camelCase or mixedCase key into snake_case."""
...
def ensure_datetime_utc(value: datetime | date | str) -> datetime:
    """Normalize a date-like value into a timezone-aware UTC datetime."""
...
def parse_polymarket_market_or_event_url(url: str) -> tuple[str, str]:
    """Extract an event-or-market kind and slug from a Polymarket URL."""
    parsed = urlparse(url)
    if not parsed.netloc:
        raise ValueError(f"Not a URL: {url!r}")

    match = _POLY_PATH.match(parsed.path or "")
    if not match:
...
def slug_variants(slug: str) -> list[str]:
    """Return Gamma-compatible candidate slugs for a Polymarket slug."""
    out = [slug]
    match = _TRAILING_NUM.match(slug)
    if match:
...
def pick_condition_id_from_markets(
    ...
    *,
    ...
    """Pick one condition id from a list of Gamma market payloads."""
    if not markets:
        raise ValueError("No markets to choose from.")

    def _get_slug(market: Mapping[str, Any]) -> str | None:
        slug = market.get("slug")
        return None if slug is None else str(slug)

    def _get_cid(market: Mapping[str, Any]) -> str | None:
        cid = market.get("conditionId") or market.get("condition_id")
        return None if cid is None else str(cid)

    if market_slug is not None:
        for market in markets:
            if _get_slug(market) == market_slug:
                cid = _get_cid(market)
                if cid:
                    return cid
    ...
    options = [(i, _get_slug(m), _get_cid(m)) for i, m in enumerate(markets)]
    raise ValueError(
        f"Multiple markets resolved; choose one via market_index or market_slug. Options: {options}"
    )
